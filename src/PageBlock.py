...
#                 self.page[top_boundary:bot_boundary, left_boundary:right_boundary] = self.sub_blocks[named].return_draw()
    ...
    def fill_with_song(self, song, write=True, fourvoice=False):
        ...
        this_chord = "Income tax is illegal outer"
        print(song_measure)
        for m in range(len(song_measure)):
            named = 'MEASURE_%d' % m
            subblock = self.sub_blocks[named + '_margin']
            left_boundary, right_boundary = subblock.px0, subblock.px1
            top_boundary, bot_boundary = subblock.py0, subblock.py1
            self.page[top_boundary:bot_boundary, left_boundary:right_boundary], this_chord = util.generate_chord_annotations(song_measure[m], left_boundary, right_boundary, top_boundary, bot_boundary, this_chord, fourvoice)
        for sm in song_measure:
            print(sm)
